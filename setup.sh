...
echo -e "${BLUE}[4/4] Checking pandoc...${NC}"
if command -v pandoc > /dev/null 2>&1; then
    echo -e "${GREEN}[4/4] pandoc is already installed${NC}"
else
    echo -e "${YELLOW}[4/4] pandoc not found${NC}"
    echo -e "${BLUE}[4/4] Installing pandoc...${NC}"
    apk add pandoc && echo -e "${GREEN}[4/4] pandoc installed successfully${NC}"
fi
